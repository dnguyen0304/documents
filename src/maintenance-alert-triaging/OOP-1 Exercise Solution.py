...
class StudentGroups(object):

    _index = list()
    _sequence_next_value = 0
    ...
    def __init__(self, name):
        ...
        if name in StudentGroups._index:
            raise ValueError
        else:
            StudentGroups._index.append(name)

        self.id = StudentGroups._sequence_next_value
        self.name = name

        self.students = list()

        StudentGroups._sequence_next_value += 1
    ...
    def __repr__(self):
        ...
        return repr_.format(class_=self.__class__.__name__, name=name)
